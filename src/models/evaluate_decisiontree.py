"""
3-Class Decision Tree — Performance Analytics Suite
...
╔════════════════════════════════════════════════════════════════════════════════════════╗
║                                                                                        ║
║     🌳  NETWORK INTRUSION DETECTION — 3-CLASS DT PERFORMANCE ANALYTICS 🌳             ║
║                                                                                        ║
║             Classes: Benign (0) | Volumetric (1) | Semantic (2)                        ║
║                                                                                        ║
╚════════════════════════════════════════════════════════════════════════════════════════╝
...
def extract_model_config(model):
    """Extract hyperparameters and metadata from the trained Decision Tree model object.

    Since train_decisiontree.py does not save a separate JSON config file,
    we build the equivalent config dict directly from the sklearn model.
    """
    ...
    config = {
        'model_type': 'DecisionTreeClassifier (3-Class)',
        'training_date': 'See model file timestamp',
        'hyperparameters': {
...
def create_hyperparameters_table(ax, config):
    """Create hyperparameters display table for Decision Tree"""

    ax.axis('off')

    hyperparams = config['hyperparameters']
    tree_stats = config.get('tree_stats', {})
    ...
    text += f"\n  {'Classification':15s}: 3-class\n"

    if tree_stats:
        text += f"\n  {'─'*33}\n"
        text += f"  {'Tree Nodes':15s}: {tree_stats.get('node_count', 'N/A')}\n"
        text += f"  {'Leaf Nodes':15s}: {tree_stats.get('leaf_count', 'N/A')}\n"
        text += f"  {'Actual Depth':15s}: {tree_stats.get('actual_depth', 'N/A')}\n"
...
✓ class_weight='balanced' used for imbalance handling
⚠ Decision Trees have limited capacity vs ensembles
⚠ Semantic class (~6%) is the hardest to detect
⚠ Consider Random Forest or XGBoost for improvement
    """
...
def create_executive_dashboard(config, metrics, reports_dir):
    """Create comprehensive executive dashboard for 3-class DT"""
    ...
    # Main title
    title = f"Network Intrusion Detection — 3-Class Decision Tree Analytics Dashboard\n"
    title += f"Model: {config['model_type']} | "
...
Weighted Avg     {metrics['weighted_precision']:.4f}      {metrics['weighted_recall']:.4f}     {metrics['weighted_f1']:.4f}      {metrics['total_samples']:,}
    ...
    # Save dashboard
    dashboard_path = os.path.join(reports_dir, "dt_3class_executive_dashboard.png")
...
def create_detailed_numeric_report(config, metrics, reports_dir):
    """Create detailed textual numeric report for 3-class Decision Tree"""
    ...
    report_text = f"""
╔════════════════════════════════════════════════════════════════════════════════════════════╗
║         NETWORK INTRUSION DETECTION — 3-CLASS DT COMPREHENSIVE NUMERICAL REPORT           ║
╚════════════════════════════════════════════════════════════════════════════════════════════╝
...
HYPERPARAMETERS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Algorithm:                   Decision Tree Classifier (3-Class)
max_depth:                   {hp.get('max_depth', 'None (unlimited)')}
criterion:                   {hp.get('criterion', 'N/A')}
class_weight:                {hp.get('class_weight', 'N/A')}
min_samples_split:           {hp.get('min_samples_split', 'N/A')}
min_samples_leaf:            {hp.get('min_samples_leaf', 'N/A')}
splitter:                    {hp.get('splitter', 'N/A')}
random_state:                {hp.get('random_state', 'N/A')}
Tree Nodes:                  {ts.get('node_count', 'N/A')}
Leaf Nodes:                  {ts.get('leaf_count', 'N/A')}
Actual Depth:                {ts.get('actual_depth', 'N/A')}
...
✓ STRENGTHS:
  • class_weight='balanced' compensates for imbalanced Semantic class
  • Fully interpretable model — tree rules can be exported as if-then-else
  • No binary threshold needed — direct 3-class predict()

⚠ CONSIDERATIONS:
  • Decision Trees have limited capacity vs ensemble methods (RF, XGBoost)
  • Semantic class (~6%) is the minority — monitor closely
  • Consider Random Forest or Gradient Boosting for higher accuracy

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Model Version: {config['model_type']}
Analysis Tool: 3-Class DT Performance Analytics Suite v1.0
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    """
    ...
    plt.tight_layout()
    report_path = os.path.join(reports_dir, "dt_3class_detailed_numeric_report.png")
...
def load_model_and_data():
    """Load Decision Tree model and test data; generate predictions"""

    print_header()

    print(f"{YELLOW}📂 Step 1: Loading Model and Test Data...{RESET}")
    ...
    # Load model (Decision Tree uses pickle, not joblib)
    model_path = os.path.join(models_dir, "dt_3class_model.pkl")
    if not os.path.exists(model_path):
        print(f"\n{RED}{'='*90}")
        print(f"❌ ERROR: Decision Tree model not found!")
        print(f"{'='*90}")
        print(f"\n📁 Missing file: {model_path}")
        print(f"\n{YELLOW}💡 SOLUTION:{RESET}")
        print(f"   1. Train the 3-class Decision Tree model first:")
        print(f"      {CYAN}python src/models/train_decisiontree.py{RESET}")
        print(f"   2. Ensure training completes successfully")
        print(f"   3. Verify dt_3class_model.pkl exists in models/ directory")
        print(f"\n{RED}{'='*90}{RESET}\n")
        return None
    ...
    # Extract config from model object (DT doesn't save a separate JSON config)
    ...
    print(f"  🌳 Max Depth:         {config['hyperparameters']['max_depth']}")
    print(f"  📐 Criterion:         {config['hyperparameters']['criterion']}")
    print(f"  ⚖️  Class Weight:      {config['hyperparameters']['class_weight']}")
    ts = config.get('tree_stats', {})
    print(f"  🔢 Tree Nodes:        {ts.get('node_count', 'N/A')}")
    print(f"  🍃 Leaf Nodes:        {ts.get('leaf_count', 'N/A')}")
    print(f"  📏 Actual Depth:      {ts.get('actual_depth', 'N/A')}")
...
def main():
    ...
    try:
        ...
        # Final summary
        print(f"\n{GREEN}{'═'*90}")
        print(f"{'🎉 3-CLASS DT ANALYSIS COMPLETE! 🎉':^90}")
        print(f"{'═'*90}{RESET}\n")

        print(f"{CYAN}📊 Generated Reports:{RESET}")
        print(f"   1. {BOLD}Executive Dashboard{RESET}:    dt_3class_executive_dashboard.png")
        print(f"   2. {BOLD}Detailed Numeric Report{RESET}: dt_3class_detailed_numeric_report.png")
